...
class Products extends React.Component {
  ...
  productAlreadyExists = (productName) => {
    ...
    this.state.products.forEach((product) => {
      if (product.name === productName) sameProductFound = true;
    });
  ...
  setProductToUpdate = (productToUpdate) => {
    ...
    if (!productFound) this.state.productsToUpdate.push(productToUpdate);
  };
  ...
  saveChanges = () => {
    ...
    if (this.state.productsToUpdate.length > 0) {
      this.state.productsToUpdate.forEach((productToUpdate) => {
        fetch(
          `http://localhost:9000/products/${productToUpdate.name}?count=${productToUpdate.count}`,
          { method: "PUT" }
        );
      });
    ...
    if (this.state.productsToDelete.length > 0) {
      this.state.productsToDelete.forEach((productToDelete) => {
        fetch(`http://localhost:9000/products/${productToDelete}`, {
          method: "DELETE",
        });
      });
